...
// writeFacing: when true, bake DirY/FullBody facing into dynamic HARA_ROT1 + MUKI
// (umin_convertor facing=True). Default false matches stock TK8 body anims.
bool ConvertT7AnimToPanm(const uint8_t* src, size_t srcLen,
                         std::vector<uint8_t>& outPanm,
                         std::string& errorMsg,
                         bool writeFacing = false);
