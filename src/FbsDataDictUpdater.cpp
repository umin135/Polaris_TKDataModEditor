// FbsDataDictUpdater.cpp
// Compares the remote fbsdatas version against the local cache and
// downloads a fresh data.json when the remote version is newer.
//
// Remote host : raw.githubusercontent.com (HTTPS)
// Remote paths:
//   /umin135/Polaris_TKDataModEditor/main/data/fbsdatas/version.json
//   /umin135/Polaris_TKDataModEditor/main/data/fbsdatas/data.json
...
// ---------------------------------------------------------------------------
//  Helpers (identical pattern to KamuiDictUpdater)
// ---------------------------------------------------------------------------

static const wchar_t* kHost    = L"raw.githubusercontent.com";
static const wchar_t* kVerPath = L"/umin135/Polaris_TKDataModEditor/main/data/fbsdatas/version.json";
static const wchar_t* kDatPath = L"/umin135/Polaris_TKDataModEditor/main/data/fbsdatas/data.json";
...
static bool WriteFileFbs(const std::string& path, const std::string& data)
{
...
{
    std::string fbsDir = resDir + "\\fbsdatas";
    CreateDirectoryA(fbsDir.c_str(), nullptr);

    std::string localVerPath  = fbsDir + "\\version.json";
    std::string localDataPath = fbsDir + "\\data.json";
    ...
    if (!WriteFileFbs(localDataPath, remoteData))  return false;
    if (!WriteFileFbs(localVerPath,  remoteVerJson)) return false;
