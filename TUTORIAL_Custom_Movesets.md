...
| **Hit Conditions** | Damage values and what happens on hit/block/whiff |
...
| Field | Description |
|---|---|
| `name_key` | Index into the move name database — determines the move's display name |
| `anim_key` | Index into the animation database — determines which animation plays |
| `ordinal_id` | Unique identifier for this move within the character's moveset |
...
| `recovery` | Number of frames of recovery after the active window ends |
| `airborne_start` | Frame where the character becomes airborne (0 = never) |
| `airborne_end` | Frame where the character lands |
| `ground_fall` | Frame where the character hits the ground |

### Hit Properties

| Field | Description |
|---|---|
| `vuln` | Vulnerability state — defines how the character can be hit during this move |
| `hitlevel` | The hit level of the attack (high, mid, low, etc.) |
| `collision` | Collision state identifier |
| `distance` | Attack range/distance value |
...
4. Modify the frame data fields:
   - **startup**: Lower this value to make the move faster. For example, changing a 15-frame startup to 12 makes the move 3 frames faster.
   - **recovery**: Lower this to reduce punishment risk. Higher values make the move more unsafe.
...
3. Link a **Requirement** that defines when this applies:
   - Requirement `43` = "On Hit"
   - Requirement `47` = "On Block"
   - Requirement `48` = "On Whiff"
4. Link a **Reaction List** for knockback behavior.
5. Set the move's `hit_condition_idx` to point to your new entry.

Terminate the hit condition list with an entry where `requirement_addr = 0` (null).

### Step 6: Create Cancels

Add entries to the Cancel block to define what this move can cancel into. See the next section for details.
...
Every cancel list must end with a terminator entry where `move_id = 0x8000`.
...
- **On-hit only cancel**: Attach a Requirement with `req = 43` (On Hit) to the cancel entry.
...
- `left_side`, `right_side` — side hits
...
| `49` | On hit or block |
| `51` | On hit |
| `1100` | **List terminator** — marks end of requirement list |
